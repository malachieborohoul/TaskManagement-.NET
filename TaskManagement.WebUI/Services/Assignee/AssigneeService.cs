...
public class AssigneeService(HttpClientService httpClientService):IAssigneeService
{
    ...
    public async Task<IEnumerable<Domain.Entity.Tasks.Assignee>> GetAssigneesByTaskIdAsync(Guid taskId)
    {
        try
        {
           // var privateClient = await httpClientService.GetPrivateClient();
           var privateClient =  await httpClientService.GetPrivateClient();
            ...
            var result = await response.Content.ReadFromJsonAsync<IEnumerable<Domain.Entity.Tasks.Assignee>>(); 
            return result!;
